...
describe('runUsage', () => {
  ...
  it('已有 usage.json 时，合并同日期数据', async () => {
    ...
    const existing = JSON.stringify({
      '2026-09-09': { input_cached: 10, input_missed: 20, output_missed: 30 },
      '2026-09-08': { input_cached: 1, input_missed: 2, output_missed: 3 }
    });
    mockReadFileSync.mockImplementation((p: string) => {
    ...
    const [, writeData] = mockWriteFileSync.mock.calls[0];
    const parsed = JSON.parse(writeData as string);
    // 2026-09-09 合并：10+200=210, 20+150=170, 0+0=0, 30+300=330
    expect(parsed['2026-09-09']).toEqual({
      input_cached: 210,
      input_missed: 170,
      output_missed: 330
  ...
  it('usage.json 按日期升序排序', async () => {
    ...
    const existing = JSON.stringify({
      '2026-09-10': { input_cached: 1, input_missed: 2, output_missed: 3 },
      '2026-09-07': { input_cached: 4, input_missed: 5, output_missed: 6 }
    });
    mockReadFileSync.mockImplementation((p: string) => {
      if ((p as string).endsWith('usage.json')) return existing;
      throw new Error('ENOENT');
  ...
  it('控制台输出今日总消耗与项目总消耗', async () => {
    ...
    const existing = JSON.stringify({
      '2026-09-08': { input_cached: 10, input_missed: 20, output_missed: 30 }
    });
    mockReadFileSync.mockImplementation((p: string) => {
      if ((p as string).endsWith('usage.json')) return existing;
      if ((p as string).endsWith('.jsonl')) return SESSION_LINE;
    ...
    expect(allOutput).toContain('output_missed: 300');
    // 总计 = 09-08 + 09-09
    ...
    expect(allOutput).toContain('output_missed: 330');
